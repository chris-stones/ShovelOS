...
void probe_clidr() {
	...
	_debug_out("clidr "); _debug_out_uint(clidr); _debug_out("\r\n");
	_debug_out("louu  "); _debug_out_uint(louu ); _debug_out("\r\n");
	_debug_out("loc   "); _debug_out_uint(loc  ); _debug_out("\r\n");
	_debug_out("louis "); _debug_out_uint(louis); _debug_out("\r\n");

	for(uint32_t index=0;index<7;index++) {

		_debug_out_uint(index);

		uint32_t ctype = (clidr >> (index*3)) & 7;

		switch(ctype) {
		default:
			_debug_out(" Unknown Cache.\r\n");
			break;
		case 0:
			_debug_out(" No Cache.\r\n");
			break;
		case 1:
			_debug_out(" Instruction Cache.\r\n");
			break;
		case 2:
			_debug_out(" Data Cache.\r\n");
			break;
		case 3:
			_debug_out(" Separate Data/Instruction Caches.\r\n");
			break;
		case 4:
			_debug_out(" Unified Cache.\r\n");
			break;
		}
	}
}
