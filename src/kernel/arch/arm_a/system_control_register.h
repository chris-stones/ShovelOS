...
typedef enum system_control_register {
	...
	SCTLR_AFE							= (1<<29),  // Access flag enable.
	SCTLR_TRE							= (1<<28),	// TEX re-map enable.
	SCTLR_NMFE                          = (1<<27),  // Non-maskable FIQ Enable.
	SCTLR_EE                            = (1<<25),  // Exception Endianness.
	SCTLR_VE							= (1<<24),  // DEPRECATED. Vector interrupt enable. 0=see 'V', 1=implementation defined.
	SCTLR_FI                            = (1<<21),  // Fast interrupt configuration enable.
